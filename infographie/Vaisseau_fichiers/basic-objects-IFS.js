// The original version of this code comes from http://math.hws.edu/eck/cs424/notes2013/19_GLSL.html
//
// Functions were added in order to be able to draw hemispheres and quarterspheres. 
// In both cases, you need to draw the outer part (outside) and the inner part (inside)
// to be able to see the reflection of light on the outer and inner surfaces.
// (Of course, a more elegant way to do this requires the use of two pairs of shaders:
//   one pair for the outside and another pair for the inside. In that case, only
//   one function drawing the hemisphere or the quartersphere is needed.)
//
/**
...
function trapezeForm(side) {
   ...
   face( [-s,-s,s, s,-s,s, s,s,s, -s,s,s], [0,0,1] );
   face( [-s/1.5,-s,-s, -s/1.5,s,-s, s/1.5,s,-s, s/1.5,-s,-s], [0,0,-1] );
   face( [-s/1.5,s,-s, -s,s,s, s,s,s, s/1.5,s,-s], [0,1,0] );
   face( [-s/1.5,-s,-s, s/1.5,-s,-s, s,-s,s, -s,-s,s], [0,-1,0] );
   face( [s/1.5,-s,-s, s/1.5,s,-s, s,s,s, s,-s,s], [1,0,0] );
   face( [-s/1.5,-s,-s, -s,-s,s, -s,s,s, -s/1.5,s,-s], [-1,0,0] );
...
function trapezeFormAngle(side) {
   var s = (side || 1)/2;
   var coords = [];
   var normals = [];
   var texCoords = [];
   ...
   face( [-s,-s,s, s,-s,s, s,s,s, -s,s,s], [0,0,1] );
   face( [-s/1.5,-s,-s, -s/1.5,s/4,-s, s/1.5,s/4,-s, s/1.5,-s,-s], [0,0,-1] );
   face( [-s/1.5,s/4,-s, -s,s,s, s,s,s, s/1.5,s/4,-s], [0,1,0] );
   face( [-s/1.5,-s,-s, s/1.5,-s,-s, s,-s,s, -s,-s,s], [0,-1,0] );
   face( [s/1.5,-s,-s, s/1.5,s/4,-s, s,s,s, s,-s,s], [1,0,0] );
   face( [-s/1.5,-s,-s, -s,-s,s, -s,s,s, -s/1.5,s/4,-s], [-1,0,0] );
...
function piedPorteFormInver(side) {
   var s = (side || 1)/2;
   var coords = [];
   var normals = [];
   var texCoords = [];
   ...
   face( [-s,-s/4,s, s,-s/4,s, s,s,s, -s,s,s], [0,0,1] );
   face( [-s,-s,-s, -s,s,-s, s,s,-s, s,-s,-s], [0,0,-1] );
   face( [-s,s,-s, -s,s,s, s,s,s, s,s,-s], [0,1,0] );
   face( [-s,-s,-s, s,-s,-s, s,-s/4,s, -s,-s/4,s], [0,-1,0] );
   face( [s,-s,-s, s,s,-s, s,s,s, s,-s/4,s], [1,0,0] );
   face( [-s,-s,-s, -s,-s/4,s, -s,s,s, -s,s,-s], [-1,0,0] );
...
function tétraèdreForm(side) {
   ...
   face( [0,s,s, s,-s,s, 0,0,-s], calculenormale([0,s,s],[s,-s,s],[0,0,-s]) );
   face( [-s,-s,s, 0,s,s, 0,0,-s], calculenormale([-s,-s,s],[0,s,s],[0,0,-s]) );
   face( [-s,-s,s, s,-s,s, 0,0,-s ], calculenormale([-s,-s,-s],[s,-s,-s],[0,0,s]) );
   face( [-s,-s,s, s,-s,s, 0,s,s], calculenormale([-s,-s,s,],[s,s,s],[-s,s,s]) );
   return {
      vertexPositions: new Float32Array(coords),
      vertexNormals: new Float32Array(normals),
      vertexTextureCoords: new Float32Array(texCoords),
...
function cockpitForm(side) {
   ...
   face( [-s,-s,s, s,-s,s, s/1.5,s,s, -s/1.5,s,s], [0,0,1] ); //bas
   face( [-s/1.5,-s,-s, -s/1.5,s/4,-s, s/1.5,s/4,-s, s/1.5,-s,-s], [0,0,-1] ); //dessus
   face( [-s/1.5,s/4,-s, -s/1.5,s,s/8, s/1.5,s,s/8, s/1.5,s/4,-s], [0,1,0] ); //devant
   face( [-s/1.5,-s,-s, s/1.5,-s,-s, s,-s,s/8, -s,-s,s/8], [0,-1,0] ); //arrière
   face( [s/1.5,-s,-s, s/1.5,s/4,-s, s/1.5,s,s/8, s,-s,s/8], [1,0,0] ); //droite
   face( [-s/1.5,-s,-s, -s,-s,s/8, -s/1.5,s,s/8, -s/1.5,s/4,-s], [-1,0,0] ); //gauche
   face( [-s,-s,s, s,-s,s, s,-s,s/8, -s,-s,s/8], [0,-1,0] ); //arrière-bas
   face( [-s/1.5,s,s/8, s/1.5,s,s/8, s/1.5,s,s, -s/1.5,s,s], [0,1,0] );
   face( [-s/1.5,s,s/8, -s,-s,s/8,-s,-s,s, -s/1.5,s,s], [-1,0,0] );
   face( [s/1.5,s,s/8, s,-s,s/8,s,-s,s, s/1.5,s,s], [1,0,0] );
...
function calculenormale(p1, p2, p3){
...
/************  THE REMAINING CODE SECTION HAS BEEN ADDED TO THE ORIGINAL FILE **************/
/**
 * Create a model of a hemisphere.  The z-axis is the axis of the sphere,
 * with the north pole on the positive z-axis and the center at (0,0,0).
 * @param radius the radius of the sphere, default 0.5 if not specified.
 * @param slices the number of lines of longitude, default 32
 * @param stacks the number of lines of latitude plus 1, default 16.  (This 
 ...
 *    north pole and the south pole.)
 */
function uvHemisphereOutside(radius, slices, stacks) {
   radius = radius || 0.5;
   slices = slices || 32;
   stacks = stacks || 16;
   var vertexCount = (slices+1)*(stacks+1);
   ...
   var indices = new Uint16Array( 2*slices*stacks*3 );
   var du = 2*Math.PI/slices;
   var dv = (Math.PI/2)/stacks;  // this increment is half of the one for the sphere
   var i,j,u,v,x,y,z;
   var indexV = 0;
   var indexT = 0;
   for (i = 0; i <= stacks; i++) {
...
function uvHemisphereInside(radius, slices, stacks) {
   radius = radius || 0.5;
   slices = slices || 32;
   stacks = stacks || 16;
   var vertexCount = (slices+1)*(stacks+1);
   ...
   for (i = 0; i <= stacks; i++) {
      ...
      for (j = 0; j <= slices; j++) {
         ...
         z = Math.sin(v);
         vertices[indexV] = radius*x;
         normals[indexV++] = -x;       //  invert component x of the normal
         vertices[indexV] = radius*y;
         normals[indexV++] = -y;       //  invert component y of the normal
         vertices[indexV] = radius*z;
         normals[indexV++] = -z;       //  invert component z of the normal
         texCoords[indexT++] = j/slices;
         texCoords[indexT++] = i/stacks;
      } 
   }
...
function uvQuartersphereOutside(radius, slices, stacks) {
   radius = radius || 0.5;
   slices = slices || 32;
   stacks = stacks || 16;
   var vertexCount = (slices+1)*(stacks+1);
   ...
   var texCoords = new Float32Array( 2*vertexCount );
   var indices = new Uint16Array( 2*slices*stacks*3 );
   var du = Math.PI/slices;    // this increment is half of the one for the sphere
   var dv = (Math.PI/2)/stacks;  // this increment is half of the one for the sphere
   var i,j,u,v,x,y,z;
   var indexV = 0;
   var indexT = 0;
...
function uvQuartersphereInside(radius, slices, stacks) {
   radius = radius || 0.5;
   slices = slices || 32;
   stacks = stacks || 16;
   var vertexCount = (slices+1)*(stacks+1);
   ...
   for (i = 0; i <= stacks; i++) {
      v = -Math.PI/2 + i*dv;        // angle v goes from -pi/2 to zero (instead of -pi/2 to pi/2 for the sphere)
      for (j = 0; j <= slices; j++) {
         u = j*du;                  // angle u goes from 0 to pi (instead of 0 to 2*pi for the sphere)
         x = Math.cos(u)*Math.cos(v);
         y = Math.sin(u)*Math.cos(v);
         z = Math.sin(v);
         vertices[indexV] = radius*x;
